			...
			: public PartBase
		{
		  public:
								 ...
								 const PartFactoryRef &factory)
			{
				...
				try {
					m_outercolor = zugster::cinderext::parse_colorA(element.getChild("OuterColor").getValue());
				} catch ( ... ) {}
				try {
					m_innercolor = zugster::cinderext::parse_colorA(element.getChild("InnerColor").getValue());
				} catch ( ... ) {}
				try {
